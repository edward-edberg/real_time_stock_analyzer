 ...
 */
import { TextContainerProperty, TextContainerUpgrade, CreateStartUpPageContainer, } from '@evenrealities/even_hub_sdk';
...
export function formatFeedItem(item) {
    ...
    const impactLines = item.analysis.impacts.map((i) => `${arrow(i.direction)} ${i.ticker.padEnd(5)} ${i.confidence.padEnd(4)} ${i.reason}`);
    const separator = '──────────────────────────';
 ...
 */
export function formatFeedItemSimple(item) {
    const impactLines = item.analysis.impacts.map((i) => `${arrow(i.direction)} ${i.ticker.padEnd(5)} ${i.confidence.padEnd(4)} ${i.reason}`);
    return impactLines.join('\n');
}
export function formatLoading() {
 ...
 */
export async function initPage(bridge, content) {
 ...
 */
export async function updatePage(bridge, content) {
