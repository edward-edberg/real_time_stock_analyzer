...
let lastItem: FeedItem | null = null

function setStatus(state: 'idle' | 'loading' | 'ok' | 'error') {
  if (!statusDot) return
...
function renderDashboard(item: FeedItem) {
  if (tweetAuthor) tweetAuthor.textContent = item.author
  if (tweetTime) {
    const m = item.timestamp.match(/T(\d{2}:\d{2})/)
    tweetTime.textContent = m ? m[1] : ''
  }
  if (tweetText) tweetText.textContent = item.tweet_text
  if (summaryText) summaryText.textContent = item.analysis.summary
...
async function sendToGlasses(content: string) {
  if (!bridge) return
...
function buildGlassesContent(item: FeedItem): string {
...
async function connectBridge() {
  ...
  if (initResult !== 0) console.error('initPage failed with code', initResult)
}
